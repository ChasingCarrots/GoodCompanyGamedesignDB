...
class SampleProduct(models.Model):
    ...
    ProductType = models.ForeignKey(ProductType, null=True)
    ...
    def getPriceMultiplier(self):
        ...
        value = (rating)**self.ProductType.BaseMarketCurvePotential
        value *= (self.ProductType.BaseMarketMaxPriceFactor - 1)
        value += 1
        return value

    def getOptimalPrice(self):
        return self.ProductType.BaseMarketPrice * self.getPriceMultiplier()
    ...
    def getFeatures(self, onlyOthers = False):
        ...
        if onlyOthers:
            for mandatory in self.ProductType.MandatoryFeatures.all():
                if mandatory.Feature in featureList:
                    featureList.remove(mandatory.Feature)
            for optional in self.ProductType.OptionalFeatures.all():
                if optional.Feature in featureList:
                    featureList.remove(optional.Feature)
            for drawbacks in self.ProductType.Drawbacks.all():
    ...
    def getOptionalRating(self):
        ...
        n = 0.0
        featureValues = self.getFeatureValues()
        for feature in self.ProductType.OptionalFeatures.all():
    ...
    def getDrawbackRating(self):
        ...
        n = 0.0
        featureValues = self.getFeatureValues()
        for feature in self.ProductType.Drawbacks.all():
            if feature.Feature.Name in featureValues:
                i += 1
                n += feature.getRatingValue(featureValues[feature.Feature.Name])
